...
require_once("include/vendor_contacts_variables.php");
...
$page_layouts["vendor_contacts_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	contact_name - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("contact_name", $data, $keylink);
			$row["contact_name_value"] = $value;
			$format = $pSet->getViewFormat("contact_name");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("contact_name")))
				$class = ' rnr-field-number';
			$row["contact_name_class"] = $class;
	//	contact_tel - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("contact_tel", $data, $keylink);
			$row["contact_tel_value"] = $value;
			$format = $pSet->getViewFormat("contact_tel");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("contact_tel")))
				$class = ' rnr-field-number';
			$row["contact_tel_class"] = $class;
	//	contact_email - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("contact_email", $data, $keylink);
			$row["contact_email_value"] = $value;
			$format = $pSet->getViewFormat("contact_email");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("contact_email")))
				$class = ' rnr-field-number';
			$row["contact_email_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("vendor_contacts", "detailspreview"));
