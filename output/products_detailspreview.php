...
require_once("include/products_variables.php");
...
$page_layouts["products_detailspreview"] = $layout;
...
if($mastertable == "product_types")
{
	$formattedValue = make_db_value("product_types_id",$_SESSION[$sessionPrefix."_masterkey1"]);
	if( $formattedValue == "null" )
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("product_types_id") . " is null";
	else
		$whereClauses[] = $pageObject->getFieldSQLDecrypt("product_types_id") . "=" . $formattedValue;
}
...
if($rowcount) 
{
	...
	//	product_name - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("product_name", $data, $keylink);
			$row["product_name_value"] = $value;
			$format = $pSet->getViewFormat("product_name");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("product_name")))
				$class = ' rnr-field-number';
			$row["product_name_class"] = $class;
	//	product_types_id - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("product_types_id", $data, $keylink);
			$row["product_types_id_value"] = $value;
			$format = $pSet->getViewFormat("product_types_id");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("product_types_id")))
				$class = ' rnr-field-number';
			$row["product_types_id_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("products", "detailspreview"));
