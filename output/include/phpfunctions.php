...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="calcalendar" && $field=="TimeField")
	{
		if ($data['TimeField']!='00:00:00')
	$value = calendar_formatTimeInDropdown( db2time("2014-01-01 ".$data["TimeField"]) );
else 
	$value = " ";;
		return $value;
	}
				if($table=="calcalendar" && $field=="EndTime")
	{
		$value = calendar_formatTimeInDropdown( db2time("2014-01-01 ".$data["EndTime"]));;
		return $value;
	}
				if($table=="caldaily" && $field=="TimeField")
	{
		if ($data['TimeField']!='00:00:00')
	$value = calendar_formatTimeInDropdown( db2time("2014-01-01 ".$data["TimeField"]) );
else 
	$value = " ";;
		return $value;
				...
				if($table=="caldaily" && $field=="EndTime")
	{
		$value = calendar_formatTimeInDropdown( db2time("2014-01-01 ".$data["EndTime"]));;
		return $value;
	}
				if($table=="caldaily" && $field=="DayEvent")
	{
		if ($value) 
	$value = "All day";;
		return $value;
	}
				if($table=="calmonthly" && $field=="TimeField")
	{
		$value = calendar_formatTimeInDropdown( db2time("2014-01-01 ".$data["TimeField"]));;
		return $value;
	}
				if($table=="calmonthly" && $field=="EndTime")
	{
		$value = calendar_formatTimeInDropdown( db2time("2014-01-01 ".$data["EndTime"]));;
		return $value;
	}
				if($table=="calmonthly" && $field=="DayEvent")
	{
		if ($value) 
	$value = "All day";;
		return $value;
	}
				if($table=="calweekly" && $field=="TimeField")
	{
		$value = calendar_formatTimeInDropdown( db2time("2014-01-01 ".$data["TimeField"]));;
		return $value;
	}
				if($table=="calweekly" && $field=="EndTime")
	{
		$value = calendar_formatTimeInDropdown( db2time("2014-01-01 ".$data["EndTime"]));;
		return $value;
	}
				if($table=="calweekly" && $field=="DayEvent")
	{
		if ($value) 
	$value = "All day";;
		return $value;
	...
	{
		if ($data['FirstDayOfWeek'] == 0)
	$value = 'Sunday';
else 
	$value = 'Monday';;
		return $value;
	}
				if($table=="calyearly" && $field=="Subject")
	{
		global $data;
if ($data["DateField2"])
	if (strtotime(now())-strtotime(runner_date_format("Y-m-d",strtotime($data["DateField2"]))." ".$data["TimeField"])>0)	
		$value="<font color=red>".$value."</font>";
if ($data["Recurrence"]==1)
	$value.="&nbsp;<img src='images\\repeat.gif' border=0>"; 
;
		return $value;
	}
	return $value;
...
function fileCustomExpression($file, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="renewals" && $field=="documents")
	{
		;
		return $value;
	}
				if($table=="license_keys" && $field=="key_files")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="caldaily" && $field=="DateField")
	{
		return $_SESSION["yr"]."-".$_SESSION["mon"]."-".$_SESSION["days"];
	}
				if($table=="caldaily" && $field=="EndDate")
	{
		return ($_SESSION["yr"])."-".$_SESSION["mon"]."-".$_SESSION["days"];
	}
				if($table=="caldaily" && $field=="Period")
	{
		return "d";
	}
				if($table=="caldaily" && $field=="PeriodDays")
	{
		return setDefaultPeriodDay();;
	}
				if($table=="calmonthly" && $field=="DateField")
	{
		return $_SESSION["yr"]."-".$_SESSION["mon"]."-".$_SESSION["days"];
	}
				if($table=="calmonthly" && $field=="EndDate")
	{
		return ($_SESSION["yrEnd"])."-".$_SESSION["monEnd"]."-".$_SESSION["daysEnd"];
	}
				if($table=="calmonthly" && $field=="Period")
	{
		return "d";
	}
				if($table=="calmonthly" && $field=="PeriodDays")
	{
		return setDefaultPeriodDay();;
	}
				if($table=="calweekly" && $field=="DateField")
	{
		return $_SESSION["yr"]."-".$_SESSION["mon"]."-".$_SESSION["days"];
	}
				if($table=="calweekly" && $field=="EndDate")
	{
		return ($_SESSION["yr"])."-".$_SESSION["mon"]."-".$_SESSION["days"];
	}
				if($table=="calweekly" && $field=="Period")
	{
		return "d";
	}
				if($table=="calweekly" && $field=="PeriodDays")
	{
		return setDefaultPeriodDay();;
	}
