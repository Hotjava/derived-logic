...
function calendar_getListOfEventsInRange($rangeFrom, $rangeTo, $searchWhereClause, $connection)
{
	...
	while( $data = $result->fetchAssoc() )
	{
		...
		$endDate = db2time( $data[ calendarGetSetting("calendarEndDate" ) ] );
			if( calendar_isEventInRange($rangeFrom, $rangeTo, $startDate, $endDate, $period ) && CheckSecurity($data["OwnerID"], "Edit", "caldaily") )
				$output[] = $data;
	}
 ...
 *		The sortEvents function relies on the records order by time of the day.
 */
function calendar_getListOfDatesInRange($rangeFrom, $rangeTo, $searchWhereClause, $connection)
{
	$calendarTable = $connection->addTableWrappers(calendarGetSetting("calendarTable"));
	$dateField = $connection->addFieldWrappers(calendarGetSetting(calendarGetDateOrTimeKey("date")));
	$fromDate = $connection->addDateQuotes( date2db( $rangeFrom ) );
	...
	while( $data = $result->fetchAssoc() )
	{
		...
		$startDate = db2time( $data[ calendarGetSetting(calendarGetDateOrTimeKey("date"))] );
		$endDate = db2time( $data[ calendarGetSetting("calendarEndDate") ] );
		if( calendar_isEventInRange($rangeFrom, $rangeTo, $startDate, $endDate, $period ) && CheckSecurity($data["OwnerID"], "Edit", "calyearly") )
			$output[] = $data;
	}
	return $output;
}
...
function calendar_tempValidateEvent(&$values, &$ret, &$message)
{
	...
	if (!$values["DateField"] || !$values["EndDate"] )
	{
	...
	else
	{
		...
		if (calendarFieldIsset(calendarGetSetting(calendarGetDateOrTimeKey("time")))){
			$dbDateTime = db2time( $values["DateField"] . " " . $values["TimeField"] );
			$dbDateTimeEnd = db2time( $values["DateField"] . " " . $values["EndTime"] );
		}
		else {
			$dbDateTime = db2time( $values["DateField"] );
			$dbDateTimeEnd = db2time( $values["DateField"] );
		}
...
function calendar_outputTimeFieldsEdit($recID){
	...
	echo "];";
	echo "var timesEnd=[{value: '',display:'Please select'}";
	...
	for ($i=0; $i<(1440/$selectTimePeriod); $i++)
	{
		...
		echo ",{value: '". calendar_time2db($timeArrayEnd)."',display:'". calendar_formatTimeInDropdown($timeArrayEnd)." (".str_format_lengthEvent($lengthEvent).")'}";
...
function calendar_AddRecord($valuesRecord){
	...
	if (calendarGetDateOrTimeKey("date")=="calendarDateTime"){
		$tblCalendar->Value[calendarGetSetting("calendarDateTime")] = $valuesRecord["DateField"]." ".$valuesRecord["TimeField"];
	}
	else {
...
function calendar_sortByDateField($event1,$event2){
	...
	if ((comparedates($ev1DateField,$ev1EndDate)==0) || (comparedates($ev2DateField,$ev1DateField) < 0))  {
		return 1;
	}
	else if ((comparedates($ev2DateField,$ev2EndDate)==0) || (comparedates($ev1DateField,$ev2DateField) < 0)){
		return -1;
	}
